...
| B3.2 | PEP 639 license metadata | `pyproject.toml`: `license = "MIT"`, `license-files = ["LICENSE"]`, deprecated classifier dropped; `uv build`-time warning gone; suite green | — | ⬜ | Reuse: n/a (metadata only) |
| B3.3 | Lockfile refresh | `uv lock --upgrade` within existing ranges; pyrate-limiter stays <4 (🔒); full suite + ruff + mypy green after | B3.2 | ⬜ | Do NOT bump range pins themselves |
...
| B4.2 | OPTIMIZE: retrieval micro-benchmark | Synthetic corpus, stub embedder: chunk→BM25→hybrid RRF timed; record numbers in RECON.md; spawn an optimization leaf ONLY if egregious (>1 s for ~200-doc corpus query, or superlinear blowup) | — | ⬜ | Measure-first; no change without before/after delta |
...
| B5.1.4 | Dispatcher stall surfacing | `run_job` except-path special-cases `BackendStalled` **before** generic: audit `backend.stalled` {job_id, error, model, stalled_after_s, call} + progress event kind="stalled" label "Backend stalled — aborted" (best-effort) + existing `job.failed` + bus `job.status` failed. Mirrors `governor.tripped` precedent at dispatcher.py:1276-1281 | B5.1.1 | ⬜ | Job fails LOUDLY with audited cause — the incident's ask |
