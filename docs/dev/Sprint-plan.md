...
This is the v1.0 canonical table. Defaults shown; per-role overrides in `chosen_models.yaml`.

| Tier | Hardware | RAM/VRAM | Default reasoning model | Quant | Inference engine | Notes |
|------|----------|----------|-------------------------|-------|------------------|-------|
| **T1 — Mini** | 16 GB unified (Mac mini M4, base Air) | 16 GB | **Qwen3-8B** | Q4_K_M (~5.5 GB) | **Ollama** | Floor; 28-35 tok/s on M4; ceiling ~13B at Q4 |
| **T2 — Workstation** | 24-32 GB unified or 24 GB VRAM (RTX 3090/4090, M3 Pro) | 24-32 GB | **Qwen3-30B-A3B** MoE | Q4_K_M (~16.5 GB) | **vLLM** or **SGLang** on NVIDIA, **MLX** on Mac | MoE: ~168 tok/s single 3090; SGLang +29% on H100 vs vLLM |
| **T3 — Studio** | 64 GB unified (Mac Studio M4 Max, M3 Ultra entry) | 64 GB | **Llama-3.3-70B** or **Qwen-3.6-35B-A3B** | Q4_K_M (~43 GB) or Q8 | **MLX** primary | Watch memory pressure on 70B Q4; spike >90% causes SSD swap |
| **T4 — Workstation+** | 128 GB unified (M3 Ultra) or 2× RTX 4090 (48 GB) or A6000 | 128 GB | **Llama-3.3-70B Q8** (~78 GB) | Q8 / bf16 | **MLX** (Mac) / **vLLM** (NVIDIA) | NVLink not strictly needed; PCIe within 2.7% |
| **T5 — Multi-GPU / Ultra** | 4-8× H100/A100, or M3 Ultra 256GB+, or Mac Studio 512GB | ≥256 GB or multi-GPU | **DeepSeek-V3.1-671B** | Q4_K_M (~405 GB) or NVFP4 | **vLLM** (NVFP4: 8033 tok/s on Blackwell) or **SGLang** | M3 Ultra 256 runs Llama-405B Q4; DeepSeek-671B Q4 needs ≥512 GB unified or multi-GPU |

**Role bindings within a tier:**

| Role | Tier preference |
|------|-----------------|
| `planner` (decomposition, framing) | floor or daily — daily preferred for reasoning |
| `researcher` (section retrieval + summary) | daily |
| `synthesizer` (final composition) | ceiling |
| `aux_context` (classifiers, lint, summarization) | floor (small/fast) |
| `embedding` | (separate) BGE-M3 |
| `reranker` | (separate) Qwen3-Reranker-0.6B |
| `escalation` | cloud (opt-in per request) |

**Tier-to-role mapping:**

- **T1:** planner=daily=ceiling (one model does all). Synthesizer = Qwen3-8B with extended thinking. Aux = same model with shorter prompts.
- **T2:** planner/researcher = Qwen3-30B-A3B MoE; synthesizer = same; aux = Qwen3-8B floor for speed.
- **T3:** planner/researcher = Qwen3-30B-A3B; synthesizer = Llama-3.3-70B; aux = Qwen3-8B.
- **T4:** planner/researcher = Llama-3.3-70B Q8 or Qwen-3.6-35B-A3B Q8; synthesizer = same; aux = Qwen3-8B.
- **T5:** planner/researcher = DeepSeek-V3.1 or top tier; synthesizer = same with reasoning-mode; aux = Qwen3-8B or Llama-3.3-8B locally.
