...
def test_selected_skills_feed_documents_into_meta(skill_lib, patched_load):
    ...
    meta = {"topic": "what is quantum", "mode": "investigate",
            "selected_skills": ["alpha"],
            dispatcher._BROKER_META_KEY: object()}  # placeholder; real broker below
...
def test_investigate_job_runs_skills_end_to_end(migrated_paths, skill_lib,
                                                patched_load):
    _write_skill(skill_lib, "alpha")
    job = dispatcher.ClaimedJob(
        id="j-skills", mode="investigate",
    ...
    try:
        ...
        meta_json = conn.execute(
            "SELECT metadata_json FROM jobs WHERE id=?", ("j-skills",)).fetchone()
        # job row may not exist (dispatcher.run_job doesn't insert), so just
        # confirm the draft was written.
        row = conn.execute(
            "SELECT body_json FROM drafts WHERE id=?", (draft_id,)).fetchone()
...
def test_empty_selected_skills_leaves_meta_unchanged(skill_lib, patched_load):
    meta = {"topic": "anything", "mode": "investigate",
            dispatcher._BROKER_META_KEY: None}
    docs = dispatcher._inject_skill_documents(meta, gateway=None)
    assert docs == []
    assert "documents" not in meta  # untouched


def test_empty_selected_skills_does_not_run_skills(skill_lib, patched_load,
                                                   monkeypatch):
    called = {"n": 0}
...
def test_thin_specialty_triggers_general_web_gap_filler(skill_lib, patched_load):
    ...
    meta = {"topic": "obscure topic", "mode": "investigate",
...
def test_no_gap_filler_when_specialty_has_results(skill_lib, patched_load):
    ...
    docs = dispatcher._run_selected_skills(
        {"topic": "q", "selected_skills": ["alpha"]},
        gateway=None, broker=broker)
    assert docs
...
def test_gap_filler_absent_general_web_no_crash(skill_lib, patched_load):
    ...
    docs = dispatcher._run_selected_skills(
        {"topic": "q", "selected_skills": ["emptyspec"]},
        gateway=None, broker=broker)
    assert docs == []
...
def _cross_skill_balanced_docs():
    ...
    support = Document(
        id="c-sup", text="The treatment is effective and safe.",
        metadata={"skill_id": "skillA", "entailment_score": 0.9})
    oppose = Document(
        id="c-opp", text="The treatment is ineffective and unsafe.",
...
def test_auto_adjudicate_flag_set_under_five_conditions():
    ...
    dispatcher._maybe_flag_auto_adjudicate(
        meta, summary, _cross_skill_balanced_docs())
...
def test_auto_adjudicate_not_set_when_single_skill():
    ...
    from lighthouse_ai.rag.chunker import Document
    docs = [
        Document(id="c1", text="The treatment is effective.",
                 metadata={"skill_id": "skillA", "entailment_score": 0.9}),
        Document(id="c2", text="The treatment is ineffective.",
