...
def test_pipeline_offline_deepdive_stages_draft(migrated_paths):
    pipe = ResearchPipeline(migrated_paths, config=PipelineConfig(offline=True, mode="deep-dive"))
    ...
    try:
        row = conn.execute(
            "SELECT status, title FROM drafts WHERE id=?", (result.draft_id,)
        ).fetchone()
        job = conn.execute(
            "SELECT status FROM jobs WHERE id IS NOT NULL ORDER BY created_at DESC LIMIT 1"
        ).fetchone()
    finally:
        conn.close()
    assert row[0] == "staged"
...
def test_pipeline_offline_quc_stages_draft(migrated_paths):
    pipe = ResearchPipeline(migrated_paths, config=PipelineConfig(offline=True, mode="quc"))
...
def test_pipeline_records_positions_and_discipline(migrated_paths):
    ...
    pipe.ingest_text(
        "d1",
        "Quantum decoherence is the main challenge. "
    ...
    # positions recorded
    n = open_db(migrated_paths.positions_db).execute("SELECT COUNT(*) FROM positions").fetchone()[0]
    assert n >= 1
    # draft has a WEP band derived from discipline
    row = (
        open_db(migrated_paths.state_db)
        .execute("SELECT wep_band, confidence FROM drafts WHERE id=?", (result.draft_id,))
        .fetchone()
    )
...
def test_cli_research_offline_stages_draft(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    r = runner.invoke(
        app, ["research", "How does alpha relate to gamma?", "--doc", str(doc), "--offline"]
    )
...
def test_cli_research_missing_doc_errors(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    r = runner.invoke(app, ["research", "q", "--doc", str(tmp_path / "nope.txt"), "--offline"])
...
def test_pipeline_picks_reranker(migrated_paths, monkeypatch):
    ...
    pipe_offline = ResearchPipeline(migrated_paths, config=PipelineConfig(offline=True))
    ...
    pipe_online = ResearchPipeline(migrated_paths, config=PipelineConfig(offline=False))
    ...
    pipe_real = ResearchPipeline(migrated_paths, config=PipelineConfig(offline=False))
...
def test_pipeline_auto_fetches_when_corpus_empty(migrated_paths):
    ...
    pipe = ResearchPipeline(
        migrated_paths, config=PipelineConfig(offline=True, auto_fetch_sources=True)
    )
...
def test_pipeline_auto_fetch_disabled_by_flag(migrated_paths):
    ...
    pipe = ResearchPipeline(
        migrated_paths, config=PipelineConfig(offline=True, auto_fetch_sources=False)
    )
