...
def test_page_properties_present(tmp_path):
    page = export_draft(
        tmp_path / "g", draft_id="d-props", title="Props",
        body_html="<p>x</p>", source_count=4, artifact_type="report",
...
def test_tags_include_lighthouse_and_extras(tmp_path):
    page = export_draft(
        tmp_path / "g", draft_id="d-tags", title="T", body_html="<p>x</p>",
        tags=["physics"],
    )
    md = page.path.read_text()
...
def test_matrix_renders_table_winner_crux(tmp_path):
    ...
    report = run_decide(
        ...
        criteria=[{"label": "speed", "weight": 2.0},
                  {"label": "simplicity", "weight": 1.0}],
    )
    page = export_draft(
        tmp_path / "g", draft_id="d-mx", title="DB choice", body_html="<p>x</p>",
        artifact_type="matrix", body_json=dataclasses.asdict(report),
    )
    md = page.path.read_text()
...
def test_timeline_renders_dated_blocks_with_properties(tmp_path):
    ...
    page = export_draft(
        tmp_path / "g", draft_id="d-tl", title="Treaty", body_html="<p>x</p>",
        artifact_type="timeline", body_json=dataclasses.asdict(report),
    )
    md = page.path.read_text()
...
def test_verdict_renders_perspectives_crux_summary(tmp_path):
    ...
    page = export_draft(
        tmp_path / "g", draft_id="d-vd", title="Debate", body_html="<p>x</p>",
        artifact_type="verdict", body_json=dataclasses.asdict(result),
    )
    md = page.path.read_text()
...
def test_table_renders_evidence_table_and_prisma(tmp_path):
    ...
    docs = [
        {"doc_id": "p1", "title": "Paper one",
         "text": "The method uses transformers. Accuracy was 0.9."},
        {"doc_id": "p2", "title": "Paper two",
         "text": "The method uses CNNs. Accuracy was 0.8."},
    ]
    report = run_survey(
        "ML methods", documents=docs, criteria=[],
        attributes=[{"label": "method", "keywords": ["method"]},
                    {"label": "accuracy", "keywords": ["accuracy"]}],
    )
    page = export_draft(
        tmp_path / "g", draft_id="d-tb", title="Survey", body_html="<p>x</p>",
        artifact_type="table", body_json=dataclasses.asdict(report),
    )
    md = page.path.read_text()
...
def test_transcript_renders_turns_with_citations(tmp_path):
    ...
    page = export_draft(
        tmp_path / "g", draft_id="d-tx", title="Ask", body_html="<p>x</p>",
        artifact_type="transcript", body_json=body_json,
    )
    md = page.path.read_text()
...
def test_report_renders_sections_with_citations(tmp_path):
    body_json = {
        ...
        "sections": [
            {"title": "Background", "sub_question": "", "body": "Y is old.",
             "citations": ["s1"], "is_load_bearing": True},
            {"title": "Mechanism", "sub_question": "", "body": "Y uses Z.",
             "citations": ["s2", "s3"], "is_load_bearing": True},
        ],
        "open_questions": ["Is Y scalable?"],
        "ruled_out": [],
    }
    page = export_draft(
        tmp_path / "g", draft_id="d-rp", title="Report", body_html="<p>x</p>",
        artifact_type="report", body_json=body_json,
    )
    md = page.path.read_text()
...
def test_digest_renders_items_with_salience(tmp_path):
    ...
    items = [
        MonitorItem(source="s", url="http://a", title="Breaking major news",
                    body="critical urgent " * 50),
        MonitorItem(source="s", url="http://b", title="Minor note", body="short"),
    ]
    report = run_monitor("News", items)
    page = export_draft(
        tmp_path / "g", draft_id="d-dg", title="Watch", body_html="<p>x</p>",
        artifact_type="digest", body_json=dataclasses.asdict(report),
    )
    md = page.path.read_text()
    assert "## Digest" in md
...
def test_falls_back_to_html_when_no_body_json(tmp_path):
    page = export_draft(
        tmp_path / "g", draft_id="d-fb", title="HTML only",
...
def test_unknown_artifact_type_falls_back(tmp_path):
    page = export_draft(
        tmp_path / "g", draft_id="d-uk", title="Weird", body_html="<p>Hello.</p>",
        artifact_type="mystery", body_json={"foo": "bar"},
    )
    md = page.path.read_text()
...
def test_structured_reexport_idempotent(tmp_path):
    ...
    r1 = run_decide("Q?", options=["A", "B"],
                    criteria=[{"label": "c", "weight": 1.0}])
    a = export_draft(
        tmp_path / "g", draft_id="d-id", title="Decision", body_html="<p>x</p>",
        artifact_type="matrix", body_json=dataclasses.asdict(r1),
    )
    r2 = run_decide("Q?", options=["A", "B", "C"],
                    criteria=[{"label": "c", "weight": 1.0}])
    b = export_draft(
        tmp_path / "g", draft_id="d-id", title="Decision", body_html="<p>y</p>",
        artifact_type="matrix", body_json=dataclasses.asdict(r2),
    )
