...
def test_dashboard_api_jobs_reflect_state_db(migrated_paths):
    ...
    try:
        conn.execute(
            ...
            (
                json.dumps(
                    {"topic": "EU AI Act", "progress": 0.6, "eta": "~2h", "depth": "Thorough"}
                ),
            ),
        )
    finally:
    ...
    assert any(j["id"] == "abc" and j["topic"] == "EU AI Act" for j in body["jobs"])
...
def test_dashboard_api_budget_reflects_governor(migrated_paths):
    ...
    assert body["budget"]["tier"] in {"green", "warn", "degrade", "local_only", "drain", "tripped"}
