...
def _chunks(n=3):
    return [Chunk(id=f"c{i}", document_id=f"d{i}",
                  text=f"Evidence sentence number {i} about metabolic health.",
                  position=i, metadata={"source": f"src{i}"}) for i in range(n)]
...
def test_snapshot_accepts_hybrid_results(migrated_paths):
    ...
    class _HR:
        def __init__(self, chunk):
            self.chunk = chunk
    written = snapshot_evidence(migrated_paths.state_db, "d-2",
                                [_HR(c) for c in _chunks(2)])
    assert written == 2
...
def test_turn_offline_is_grounded_and_honest():
    ...
    res = chat_turn(session, "What does the evidence say about metabolic health?",
                    _chunks(3), gateway=None)
...
def test_turn_reports_real_backend():
    ...
    res = chat_turn(session, "What is the established finding on metabolic health?",
                    _chunks(3), gateway=_RealishGateway())
...
def test_turn_does_not_let_mock_masquerade():
    ...
    session = QUCSession(id="chat-d1", topic="t")
    res = chat_turn(session, "What is the established finding on metabolic health?",
                    _chunks(3), gateway=_MockMasqueradeGateway())
...
def test_escalation_fires_on_thin_retrieval():
    ...
    extra = [Chunk(id="new1", document_id="n1", text="Freshly acquired evidence.",
                   position=0, metadata={"source": "web"})]

    def _acquire(_q):
        return extra

    session = QUCSession(id="chat-d1", topic="t")
    res = chat_turn(session, "An entirely unrelated question about quantum gravity?",
                    [], gateway=None, acquire_fn=_acquire)
...
def test_no_escalation_when_grounding_sufficient():
    ...
    res = chat_turn(session, "What does the evidence say about metabolic health?",
                    _chunks(6), gateway=None, acquire_fn=_acquire)
    assert res.researched is False


def test_suggestions_from_artifact_open_questions():
    art = {"open_questions": ["Does timing matter?", "What about long-term?"],
