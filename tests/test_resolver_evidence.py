...
def _retriever(text="As of the date, the agency published the approval.",
               src="https://example.gov/doc"):
...
def test_defers_without_criterion():
    r = R.attempt_auto_resolve(1, "Will the drug be approved?", 0.7,
                               criterion=None, retriever=_retriever(),
...
def test_defers_human_claim():
    r = R.attempt_auto_resolve(1, "Should we invest in nuclear energy?", 0.6,
                               criterion="net-positive ROI", retriever=_retriever(),
                               gateway=_Gateway("TRUE: 0.9 — yes"))
...
def test_defers_offline_no_evidence():
    ...
    # memory-based guess.
    r = R.attempt_auto_resolve(1, "Did the agency approve the drug by 2026?", 0.7,
                               criterion="FDA approval notice exists",
                               retriever=_NONE_RETRIEVER, gateway=_Gateway("TRUE: 0.99 — yes"))
    assert r.auto_resolved is False and r.outcome is None
...
def test_defers_when_no_gateway():
    r = R.attempt_auto_resolve(1, "Did the agency approve the drug by 2026?", 0.7,
                               criterion="approval notice", retriever=_retriever(),
                               gateway=None)
...
def test_resolves_from_evidence_records_provenance():
    r = R.attempt_auto_resolve(
        1, "Did the agency approve the drug by 2026?", 0.8,
        criterion="An official approval notice exists",
        retriever=_retriever("The agency issued an approval notice in March 2025.",
                             "https://example.gov/approval"),
...
def test_defers_on_uncertain_or_lowconf():
    unc = R.attempt_auto_resolve(1, "Did X happen by 2026?", 0.7,
                                 criterion="X recorded", retriever=_retriever(),
                                 gateway=_Gateway("UNCERTAIN: — evidence unclear"))
    assert unc.auto_resolved is False
    low = R.attempt_auto_resolve(1, "Did X happen by 2026?", 0.7,
                                 criterion="X recorded", retriever=_retriever(),
...
def test_run_pass_resolves_and_defers(migrated_paths):
    ...
    p1 = record_position(db, claim="Did the agency approve the drug by 2026?",
                         probability=0.8, resolve_by="2000-01-01T00:00:00",
                         resolution_criterion="An approval notice exists")
    p2 = record_position(db, claim="Will it rain a lot next year?",
                         probability=0.6, resolve_by="2000-01-01T00:00:00")
    results = R.run_resolver_pass(
        db, gateway=_Gateway("TRUE: 0.9 — confirmed by the notice"),
    ...
    row = open_db(db).execute(
        "SELECT outcome, resolver_kind, resolved_via FROM positions WHERE id=?",
        (p1.id,)).fetchone()
