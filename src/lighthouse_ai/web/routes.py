...
def attach_web(app: FastAPI, paths: Paths, bus: EventBus | None = None) -> EventBus:
    ...
    app.mount("/ui", _NoCacheStatic(directory=str(_static_root()), html=True), name="ui")
    return bus
...
def _read_jobs(paths: Paths) -> list[dict[str, Any]]:
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT id, mode, status, metadata_json FROM jobs ORDER BY updated_at DESC LIMIT 20"
        ).fetchall()
    except Exception:
        return []
    finally:
...
def _read_calibration(paths: Paths) -> dict[str, Any]:
    if not paths.positions_db.exists():
        return {"brier": 0.0, "delta": 0.0, "resolved": 0, "pending": 0, "domains": [], "trend": []}
    try:
...
def _build_alerts(paths: Paths, dashboard: dict[str, Any]) -> list[dict[str, str]]:
    ...
    try:
        ...
        if fired:
            latest = fired[0]
            text = (
                f"{len(fired)} watched page(s) changed — latest: "
    ...
    try:
        ...
        if queue:
            alerts.append(
                {
                    "kind": "positions",
                    "text": f"{len(queue)} prediction(s) need your call — resolve them in Track",
                    "topic": "",
                }
            )
    except Exception:
