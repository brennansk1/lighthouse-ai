    ...
    if v == "limited":
        if not verdict.reachable:
            ...
            return {
                ...
                "detail": (
                    "Add this site to your trusted list and Lighthouse can start watching it."
                ),
                "change_method": verdict.change_method,
                "needs_trust": True,
...
def register_api(app: FastAPI, paths: Paths, bus: EventBus) -> None:
    ...
    def list_jobs(status: str | None = None, limit: int = 50) -> dict[str, Any]:
        ...
        try:
            if status:
                ...
                rows = _rows(conn, "SELECT * FROM jobs ORDER BY updated_at DESC LIMIT ?", (limit,))
        finally:
            conn.close()
        for r in rows:
    ...
    def get_job_trace(job_id: str) -> dict[str, Any]:
        ...
        return {"job_id": job_id, "status": status, "pct": pct, "events": events}
    ...
    def create_job(body: NewJob) -> dict[str, Any]:
        ...
        meta = {"topic": body.topic, "progress": 0.0, "depth": depth, "eta": "queued"}
        if body.budget:
    ...
    def list_drafts(status: str | None = None) -> dict[str, Any]:
        ...
        try:
            if status:
                rows = _rows(
                    conn,
                    "SELECT id, job_id, topic, title, wep_band, "
    ...
    def _artifact_body(draft_id: str) -> dict[str, Any]:
        conn = open_db(paths.state_db)
        try:
            rows = _rows(conn, "SELECT body_json FROM drafts WHERE id=?", (draft_id,))
        finally:
            conn.close()
        if not rows:
    ...
    def _chat_acquire_fn(gateway):
        ...
        question using recommended skills and the default sandbox broker.
        Falls back to arXiv if no documents are retrieved. Returns None when
        a gateway isn't available (offline → no escalation)."""
        if gateway is None:
            return None
        ...
        def _acquire(query: str):
            try:
                ...
                recs = recommend(query, mode="investigate", depth="quick", gateway=gateway)
                skill_ids = [r.skill_id for r in recs[:2]] if recs else ["general_web"]
                ...
                for sid in skill_ids:
                    try:
                        ...
                        run = run_skill(skill, query, broker=broker, gateway=gateway, max_results=2)
                        if run.documents:
                            docs.extend(run.documents)
                    except Exception:
                        continue
                ...
                chunks = []
                for d in docs:
                    for c in chunk_document(Document(id=d.id, text=d.text, metadata=d.metadata)):
                        chunks.append(c)
                return chunks[:8]
            except Exception:
                return []
    ...
    def get_artifact_chat(draft_id: str) -> dict[str, Any]:
        ...
        if session is not None:
            for t in session.history:
                citations_rich = (
                    list(t.citations_rich) if getattr(t, "citations_rich", None) else []
                )
                if not citations_rich and t.citations and t.role == "assistant":
                    from ..modes.artifact_chat import _citation_view, load_evidence_chunks

                    chunks = load_evidence_chunks(paths.state_db, draft_id)
                    citations_rich = _citation_view(t.citations, chunks)
                turns.append(
                    {
                        "role": t.role,
                        "text": t.text,
                        "citations": citations_rich if t.role == "assistant" else [],
                        "backend": getattr(t, "backend", None),
                        "researched": getattr(t, "researched", None),
                        "research_note": getattr(t, "research_note", None),
                        "wep_band": getattr(t, "wep_band", None),
                    }
                )
        return {
            "session_id": session_id_for(draft_id),
            "turns": turns,
    ...
    def post_artifact_chat(draft_id: str, body: ChatBody) -> dict[str, Any]:
        ...
        if gateway is not None:
            try:
                ...
                embedder, _, _ = make_embedder(offline=False)
            except Exception:
                embedder = None
        ...
        bus.publish(
            "chat.turn",
            {"draft_id": draft_id, "backend": result.backend, "researched": result.researched},
        )
        return payload
    ...
    def approve_draft(draft_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            conn.execute(
                ...
                "WHERE id = (SELECT job_id FROM drafts WHERE id=?)",
                (draft_id,),
            )
        finally:
            conn.close()
    ...
    def get_topic(topic_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            sources = _rows(conn, "SELECT * FROM sources WHERE topic_id=? ORDER BY id", (topic_id,))
        finally:
            conn.close()
        topic = rows[0]
    ...
    def create_topic(body: NewTopic) -> dict[str, Any]:
        ...
        try:
            conn.execute(
                "INSERT INTO topics (id, name, mode, cadence) VALUES (?,?,?,?)",
                (tid, body.name, body.mode, body.cadence),
            )
            for url in body.sources:
                conn.execute("INSERT INTO sources (topic_id, url) VALUES (?, ?)", (tid, url))
        finally:
            conn.close()
    ...
    def watch_verify(body: WatchVerifyBody) -> dict[str, Any]:
        ...
        verdict = verify_scrapable(url, fetch=fetch, allowlist=DEFAULT_ALLOWED_DOMAINS)
    ...
    def watch_web_alerts(monitor_id: str | None = None, limit: int = 50) -> dict[str, Any]:
        ...
        return {
            "alerts": list_web_monitor_alerts(
    ...
    def list_positions(overdue: bool = False, resolved: bool | None = None) -> dict[str, Any]:
    ...
    def resolve(position_id: int, body: ResolveBody) -> dict[str, Any]:
        ...
        bus.publish(
            "position.resolved", {"id": position_id, "outcome": outcome, "brier": pos.brier}
        )
    ...
    def audit(limit: int = 100, event_type: str | None = None) -> dict[str, Any]:
        ...
        try:
            if event_type:
                rows = _rows(
                    conn,
                    "SELECT seq, ts, actor, event_type, payload_json "
    ...
    def governor_kill() -> dict[str, Any]:
        ...
        try:
            conn.execute(
                "UPDATE supervisor_state SET status='kill_switched', "
    ...
    def factory_reset_endpoint(body: ResetBody) -> dict[str, Any]:
        ...
        if body.confirm != "RESET":
            ...
            raise HTTPException(
                status_code=400, detail='Reset not confirmed — send {"confirm": "RESET"}.'
            )
    ...
    def update_escalation_status(escalation_id: str, body: StatusBody) -> dict[str, Any]:
        ...
        bus.publish(
            "intelligence.escalation_updated",
    ...
    def classify(q: str) -> dict[str, Any]:
        ...
        return {"question_type": qtype, "suggested_tier": auto_tier(qtype), "sub_questions": subs}
    ...
    def recommend_sources(
        q: str, mode: str = "investigate", depth: str | None = None
    ) -> dict[str, Any]:
        ...
        return {
            "recommended": [
                {"skill_id": r.skill_id, "score": r.score, "reason": r.reason, "role": r.role}
                for r in recs
            ]
        }
    ...
    def sandbox_config_set(body: SandboxConfig) -> dict[str, Any]:
        ...
        _sandbox_config_path.write_text(json.dumps({"max_bytes": body.max_bytes}), encoding="utf-8")
        store = _sandbox_store()
    ...
    def library(type: str | None = None, status: str | None = None) -> dict[str, Any]:
        clauses, params = [], []
        if type:
        ...
        try:
            rows = _rows(
                ...
                "wep_phrase, confidence, source_count, status, created_at "
                f"FROM drafts{where} ORDER BY created_at DESC",
                tuple(params),
            )
        finally:
            conn.close()
    ...
    def export_artifact(artifact_id: str, format: str = "json"):
        ...
        if format == "md":
            ...
            if body:
                try:
                    ...
                    lines = _render_body(art.get("artifact_type"), body, art.get("body_html", ""))
                    if lines:
                        md_body = "\n".join(lines)
                except Exception:
                    pass
        ...
        if format == "csv":
            return PlainTextResponse(_artifact_to_csv(art, body), media_type="text/csv")
    ...
    def calibration_timeline(bucket: str = "week") -> dict[str, Any]:
        ...
        return {"bucket": bucket, "buckets": timeline(paths.positions_db, bucket=bucket)}
    ...
    def get_draft_graph(draft_id: str) -> dict[str, Any]:
        ...
        if isinstance(sections, list):
            for i, s in enumerate(sections):
                if not isinstance(s, dict):
                    continue
                combined = " ".join(
                    filter(None, [s.get("title", ""), s.get("sub_question", ""), s.get("body", "")])
                )
        ...
        if isinstance(table_rows, list):
            for i, r in enumerate(table_rows):
                if not isinstance(r, dict):
                    continue
                cells_text = " ".join(
                    str(c.get("value", "")) for c in (r.get("cells") or []) if isinstance(c, dict)
                )
                combined = " ".join(
        ...
        verdict_fields = ("finding", "steelman", "critique", "summary", "verdict", "recommendation")
        ...
        if isinstance(perspectives, list):
            for i, p in enumerate(perspectives):
                if not isinstance(p, dict):
                    continue
                combined = " ".join(
                    filter(None, [p.get("label", ""), p.get("argument", ""), p.get("body", "")])
                )
...
def _artifact_to_csv(art: dict[str, Any], body: Any) -> str:
    ...
    if isinstance(body, dict) and isinstance(body.get("rows"), list):
        ...
        attrs = [a["label"] if isinstance(a, dict) else str(a) for a in body.get("attributes", [])]
        w.writerow(["doc_id", "title", *attrs])
        for row in body["rows"]:
            cells = {c["attribute"]: c["value"] for c in row.get("cells", [])}
            w.writerow(
                [row.get("doc_id", ""), row.get("title", ""), *[cells.get(a, "") for a in attrs]]
            )
    elif isinstance(body, dict) and isinstance(body.get("cells"), list) and "totals" in body:
        # Decide matrix: option x criterion scores.
        w.writerow(["option", "criterion", "score", "contribution"])
        for c in body["cells"]:
            w.writerow([c.get("option"), c.get("criterion"), c.get("score"), c.get("contribution")])
    elif isinstance(body, dict) and isinstance(body.get("events"), list):
        # Reconstruct timeline.
        w.writerow(["date", "action", "sources", "certainty"])
        for e in body["events"]:
            w.writerow(
                [e.get("date"), e.get("action"), ";".join(e.get("sources", [])), e.get("certainty")]
            )
    else:
        w.writerow(["title"])
...
def _build_health(paths: Paths) -> dict[str, Any]:
    ...
    try:
        ...
        resolved = (
            resolve_against_installed(profile, installed, budget_gb=budget) if installed else {}
        )
    ...
    subdirs = {
        name: _dir_size(getattr(paths, attr))
        for name, attr in [
            ("corpus", "corpus_dir"),
    ...
    return {
        ...
        "hardware": {
            ...
            "gpu": [
                {"name": g.name, "vram_gb": g.vram_gb, "vendor": g.vendor} for g in profile.gpu
            ],
        },
