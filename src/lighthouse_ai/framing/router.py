...
_DATE_PATTERNS = [
    ...
    re.compile(
        r"\b(today|yesterday|this week|this month|recent|latest|"
        r"current|now)\b",
        re.IGNORECASE,
    ),
]
_RELATIONAL_PATTERNS = [
    re.compile(
        r"\b(between|across|compared with|relationship|"
        r"impact of .* on|how does .* affect)\b",
        re.IGNORECASE,
    ),
]
_NO_RETRIEVAL_HINTS = re.compile(r"^(define|what is|what does|explain) +[a-z ]+\?$", re.IGNORECASE)
...
class AdaptiveRouter:
    ...
    def _classify_llm(
        self, query: str, *, qtype: QuestionType | None, gateway, job_id: str | None
    ) -> AdaptiveRoute | None:
        ...
        try:
            ...
            for part in re.split(r"[\s:>-]+", token):
                if part in _VALID_ROUTE_VALUES:
                    return AdaptiveRoute(RouteKind(part), "planner LLM few-shot route", 0.8)
            return None
        except Exception:
            return None
    ...
    def _classify_rules(self, query: str, *, qtype: QuestionType | None = None) -> AdaptiveRoute:
        ...
        if _NO_RETRIEVAL_HINTS.match(text) and len(text.split()) <= 6:
            return AdaptiveRoute(
                RouteKind.NONE, "short definition question — parametric likely sufficient", 0.6
            )

        # Recency cues short-circuit to recency-weighted retrieval.
        if any(p.search(text) for p in _DATE_PATTERNS):
            return AdaptiveRoute(RouteKind.RECENCY, "date or recency cue present", 0.75)

        # Comparative / multi-entity → graph or agentic.
        if qtype is QuestionType.COMPARATIVE:
            return AdaptiveRoute(
                RouteKind.AGENTIC, "comparative typed question — multi-step lookup", 0.7
            )
        if any(p.search(lowered) for p in _RELATIONAL_PATTERNS):
            return AdaptiveRoute(
                RouteKind.GRAPH, "relational language — graph synthesis indicated", 0.65
            )

        # Long, exploratory → agentic ReAct.
        if len(text.split()) > 16:
            return AdaptiveRoute(
                RouteKind.AGENTIC, "long exploratory query — agentic loop indicated", 0.55
            )

        # Default.
        return AdaptiveRoute(RouteKind.VECTOR, "default single-shot dense+sparse retrieval", 0.5)
