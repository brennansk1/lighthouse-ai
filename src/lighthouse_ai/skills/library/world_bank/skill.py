"""World Bank skill entrypoints — ``run``.

All network access is delegated to the vetted
``lighthouse_ai.sources.world_bank`` adapter; no httpx/requests/urllib/socket
imports are permitted in skill files and the registry import guard enforces
this statically.

**Egress note:** ``api.worldbank.org`` is NOT on the default Lighthouse platform
...
from lighthouse_ai.sources import world_bank as _world_bank
...
def run(
    ...
    *,
    ...
    """Research a question using the World Bank Open Data API.

    Searches World Bank indicators matching the question and wraps each
    ...
    **Egress degradation:** Any exception (including EgressBlocked) is caught;
    ...
    """
    try:
        raw_docs = _world_bank.search_indicator(question, max_results=max_results)
    except Exception as exc:
        log.info(
            "world_bank.skill.egress_blocked_or_error",
            error=repr(exc),
            note="run `lighthouse trust add api.worldbank.org` to enable live fetches",
        )
        return []
